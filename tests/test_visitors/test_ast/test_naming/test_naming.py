...
def test_anonymous_variable_name(
    assert_errors,
    assert_error_text,
    parse_ast_tree,
    naming_template,
    ...
    """Ensures that anonymous variables are not allowed."""
    variable_name = '_'
    tree = parse_ast_tree(mode(naming_template.format(variable_name)))

    visitor = WrongNameVisitor(default_options, tree=tree)
    visitor.run()

    assert_errors(visitor, [AnonymousVariableViolation])
